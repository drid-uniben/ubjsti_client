...
const [conflictingReviews, setConflictingReviews] = useState<ConflictingReview[]>([]);
  ...
  useEffect(() => {
    const fetchReviewData = async () => {
      ...
      try {
        setLoading(true);
        const response = await adminReviewApi.getAdminReviewById(reviewId);
        ...
        if (review.reviewType === 'reconciliation') {
          const reconData = await adminReviewApi.getReconciliationData(reviewId);
  ...
  const handleSaveProgress = async () => {
    ...
    try {
      ...
      const response = await adminReviewApi.saveAdminReviewProgress(reviewData._id, progressData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await adminReviewApi.submitAdminReview(reviewData._id, {
        scores,
        comments: {
          commentsForAuthor,
          confidentialCommentsToEditor: confidentialComments || undefined,
...
export default AdminManuscriptReviewForm;