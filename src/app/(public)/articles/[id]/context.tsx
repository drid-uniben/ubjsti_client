...
export default function ArticleDetailPage() {
  ...
  const citations = {
    APA: articleData?.doi
      ? `Johnson, A. O., & Okeke, C. M. (2025). Decolonizing Legal Education in West Africa: A Critical Analysis of Pedagogical Approaches. UNIBEN Journal of Science, Technology and Innovation, ${articleData.volume.volumeNumber}(${articleData.issue.issueNumber}), ${articleData.pages?.start}-${articleData.pages?.end}. https://doi.org/${articleData.doi}`
      : "",
    MLA: articleData?.doi
      ? `Johnson, Afolabi O., and Chinwe M. Okeke. "Decolonizing Legal Education in West Africa: A Critical Analysis of Pedagogical Approaches." UNIBEN Journal of Science, Technology and Innovation, vol. ${articleData.volume.volumeNumber}, no. ${articleData.issue.issueNumber}, ${new Date(articleData.publishDate).getFullYear()}, pp. ${articleData.pages?.start}-${articleData.pages?.end}.`
      : "",
    ...
    Harvard: articleData?.doi
      ? `Johnson, A.O. and Okeke, C.M., ${new Date(articleData.publishDate).getFullYear()}. Decolonizing Legal Education in West Africa: A Critical Analysis of Pedagogical Approaches. UNIBEN Journal of Science, Technology and Innovation, ${articleData.volume.volumeNumber}(${articleData.issue.issueNumber}), pp.${articleData.pages?.start}-${articleData.pages?.end}.`
      : "",
  };
